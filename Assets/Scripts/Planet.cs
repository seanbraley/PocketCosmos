...
public class Planet : MonoBehaviour {
	...
	public static float MIN_SIZE = 8f;
	public static float MAX_SIZE = 12f;
	...
	public static float ORBIT_CONSTANT = 250;
	...
	void HandleMovement () {
		...
		if (Camera.main.orthographicSize < 1000) {
			...
			if (orbitPath.isVisible) {
				// Orbit the parent.
				transform.RotateAround (orbitParent.transform.position, Vector3.back, orbitSpeed * Time.deltaTime);
	 ...
	 */
	void DrawOrbit () {
		float radius = Vector3.Distance(orbitParent.transform.position,transform.position);
		...
		for (int i = 0; i < ORBIT_PATH_SEGMENTS + 1; i++) {
			...
			Vector3 pos = new Vector3(x, y, 0) + orbitParent.transform.position;
